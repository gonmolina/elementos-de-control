...
```{figure} fig1.png
:width: 300px
:alt: Diagrama en bloques de un sistema con un parámetro variable $k$
...
```{figure} fig2.png
:width: 600px
:alt: Diagrama en bloques de los sistema $G_1(s)$ y $G_2(s)$
...
```{figure} fig3.png
:width: 400px
:alt: Sistema genérico a lazo cerrado
...
```{figure} fig4.png
:width: 300px
:alt: Diagrama equivalente del sistema a lazo cerrado
...
Consideremos $G(s)= \dfrac{1}{s+10}$ y $D(s)= k \dfrac{(s+20)}{s}$ donde $k$ es el parámetro variable, para este problema nos interesa determinar los polos y ceros del sistema a lazo cerrado y compararlos con los de la funciones a lazo abierto, por lo que consideraremos que $k=2$
...
```{figure} fig5.png
:width: 250px
:alt: Vector $\overrightarrow{s}= \sigma + j \omega$
...
```{figure} fig7.png
:width: 250px
:alt: Módulo y argumento de $F(s_0)$
...
```{figure} fig8.png
:width: 500px
:alt: $G(s)$ a lazo cerrado con un controlador proporcional $D(s)=k$
...
```{figure} fig9.png
:width: 500px
:alt: Lugar de las raíces de $G(s)$
...
```{figure} fig10.png
:width: 500px
:alt: $G(s)$ en un punto del plano-s genérico $s_0$
